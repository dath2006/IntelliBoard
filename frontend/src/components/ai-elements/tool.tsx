...
export const Tool = ({ className, ...props }: ToolProps) => (
  <Collapsible
    className={cn(
      'group not-prose mb-2 w-full rounded-md border border-border/60 bg-card/50 text-xs',
      className,
    )}
    {...props}
  />
...
export const getStatusBadge = (status: ToolPart['state']) => (
  <Badge className="gap-1 rounded-full text-[10px] px-1.5 py-0 h-4" variant="secondary">
    {statusIcons[status]}
...
export const ToolContent = ({ className, ...props }: ToolContentProps) => (
  <CollapsibleContent
    className={cn(
      'data-[state=closed]:fade-out-0 data-[state=closed]:slide-out-to-top-2 data-[state=open]:slide-in-from-top-2 space-y-3 p-3 border-t border-border/40 bg-muted/30 outline-none data-[state=closed]:animate-out data-[state=open]:animate-in text-xs',
      className,
    )}
    {...props}
  />
...
export const ToolOutput = ({ className, output, errorText, ...props }: ToolOutputProps) => {
  ...
  return (
    <div className={cn('space-y-1.5', className)} {...props}>
      ...
      >
        {errorText && <div className="p-1.5">{errorText}</div>}
        {Output}
      </div>
    </div>
  );
