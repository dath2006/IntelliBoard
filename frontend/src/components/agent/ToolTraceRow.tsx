...
const TOOL_META: Record<string, ToolMeta> = {
  ...
  add_board: {
    ...
    category: 'circuit',
    icon: '🖥',
    description: (i) => `${i.board_kind ?? i.boardKind ?? ''}${i.board_id ? ` (${i.board_id})` : ''}`,
  },
  change_board_kind: {
    label: 'Change Board',
    category: 'circuit',
  ...
  add_component: {
    ...
    category: 'circuit',
    icon: '➕',
    description: (i) => `${i.metadata_id ?? i.metadataId ?? ''} → ${i.component_id ?? i.componentId ?? ''}`,
  },
  update_component: {
  ...
  route_wire: {
    ...
    category: 'circuit',
    icon: '〰️',
    description: (i) => `${i.wire_id ?? i.wireId ?? ''} (${Array.isArray(i.waypoints) ? i.waypoints.length : 0} waypoints)`,
  },
  // Code / file
  create_file: {
    label: 'Create File',
  ...
  replace_file_range: {
    ...
    category: 'code',
    icon: '✏️',
    description: (i) => `${i.file_name ?? i.fileName ?? ''} L${i.start_line ?? i.startLine}–${i.end_line ?? i.endLine}`,
  },
  apply_file_patch: {
    label: 'Patch File',
    category: 'code',
...
const CATEGORY_COLORS: Record<ToolCategory, { bg: string; border: string; text: string; dot: string }> = {
  circuit: { bg: 'rgba(56,189,248,0.07)', border: 'rgba(56,189,248,0.25)', text: '#7dd3fc', dot: '#38bdf8' },
  code:    { bg: 'rgba(167,139,250,0.07)', border: 'rgba(167,139,250,0.25)', text: '#c4b5fd', dot: '#a78bfa' },
  inspect: { bg: 'rgba(251,191,36,0.07)', border: 'rgba(251,191,36,0.2)', text: '#fcd34d', dot: '#fbbf24' },
  compile: { bg: 'rgba(52,211,153,0.07)', border: 'rgba(52,211,153,0.25)', text: '#6ee7b7', dot: '#34d399' },
  library: { bg: 'rgba(251,146,60,0.07)', border: 'rgba(251,146,60,0.2)', text: '#fdba74', dot: '#fb923c' },
  validate:{ bg: 'rgba(74,222,128,0.07)', border: 'rgba(74,222,128,0.2)', text: '#86efac', dot: '#4ade80' },
  system:  { bg: 'rgba(148,163,184,0.07)', border: 'rgba(148,163,184,0.2)', text: '#94a3b8', dot: '#64748b' },
};
...
function OutputSummary({ output }: { output: unknown }) {
  ...
  if (typeof output === 'object' && !Array.isArray(output)) {
    ...
    return (
      <div className="tool-trace__output-summary">
        {ok !== undefined && (
          <span className={`tool-trace__ok-badge ${ok ? 'tool-trace__ok-badge--ok' : 'tool-trace__ok-badge--fail'}`}>
            {ok ? '✓ ok' : '✗ failed'}
          </span>
        )}
        ...
        {pinNames && (
          <span className="tool-trace__pin-list">
            {available === false
              ? 'not available yet'
              : pinNames.slice(0, 8).join(', ') + (pinNames.length > 8 ? ` +${pinNames.length - 8}` : '')}
          </span>
        )}
      </div>
    );
  ...
  if (Array.isArray(output)) {
    return <span className="tool-trace__output-msg">{output.length} item{output.length !== 1 ? 's' : ''}</span>;
  }
...
export const ToolTraceRow: React.FC<ToolTraceRowProps> = ({ toolName, input, output, createdAt, failed }) => {
  const [expanded, setExpanded] = useState(false);
  const meta = getMeta(toolName);
  const colors = CATEGORY_COLORS[meta.category];
  const desc = input && typeof input === 'object' ? meta.description(input as Record<string, unknown>) : '';
  const isPending = output === undefined;

  return (
    <div
      className={`tool-trace${isPending ? ' tool-trace--pending' : ''}`}
      style={{ '--tool-bg': colors.bg, '--tool-border': colors.border, '--tool-text': colors.text, '--tool-dot': colors.dot } as React.CSSProperties}
    >
        ...
        <div className="tool-trace__body">
          {input !== undefined && input !== null && (
            <div className="tool-trace__section">
              ...
              <pre className="tool-trace__pre">{prettyJson(input)}</pre>
            </div>
          )}
          {output !== undefined && output !== null && (
            <div className="tool-trace__section">
              <div className="tool-trace__section-label">Output</div>
              <pre className="tool-trace__pre">{prettyJson(output)}</pre>
            </div>
          )}
        </div>
      )}
