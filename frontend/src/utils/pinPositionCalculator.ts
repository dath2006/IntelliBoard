...
import { resolvePinFromPinInfo } from './componentResolver';
...
export function calculatePinPosition(
  ...
  const pin = resolvePinFromPinInfo(pinInfo, pinName, { boardKind, componentId });
  ...
  if (typeof pin.x !== 'number' || typeof pin.y !== 'number') {
    console.warn(`[pinPositionCalculator] Pin ${pinName} has invalid coordinates on ${componentId}`);
    return null;
  }
